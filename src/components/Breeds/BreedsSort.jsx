...
const Sort = () => {
  const { currentBreedCon, limitCon, breedsCon, orderCon } =
    useContext(BreedContext);

  const [limit, setLimit] = limitCon;
  const [order, setOrder] = orderCon;
  const [breeds, setBreeds] = breedsCon;
  const [currentBreed, setCurrentBreed] = currentBreedCon;
  const { handleFilterClick, changeOrder } = FilterBreeds();
  // let label;
  // if (currentBreed.name === '' || currentBreed.name === undefined) {
  //   label = 'All Breeds';
  // } else {
  //   label = currentBreed.name;
  // }
  ...
  return (
    <StyleDiv>
      <OptionItem
        ...
        gridArea="breed"
      />
      <OptionItem ml xs align gray title="Limit: 10" content={limitLabel} />
